...
@linq mens_mail = male_df |> where(:treatment .== 1)

## (b)メールが配信されなかったグループの購買データを得る
@linq no_mail = male_df |> where(:treatment .== 0)
...
## byだけでできる
@linq summary_by_segment_biased =
  biased_data |> by(
    :treatment,
...
## (a)男性向けメールが配信されたグループの購買データを得る
@linq mens_mail_biased = biased_data |> where(:treatment .== 1)

## (b)メールが配信されなかったグループの購買データを得る
@linq no_mail_biased = biased_data |> where(:treatment .== 0)

## (a)(b)の平均の差に対して有意差検定を実行
rct_ttest_biased =
  EqualVarianceTTest(mens_mail_biased[:, :spend], no_mail_biased[:, :spend])
